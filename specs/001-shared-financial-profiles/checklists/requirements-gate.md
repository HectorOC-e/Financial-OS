...
- [ ] CHK001 Are requirements defined for the invitation lifecycle (sending, accepting, declining, and expiry of an invite), not just the act of inviting? [Completeness, Gap, Spec §FR-002]
- [ ] CHK002 Is the source and update mechanism of a member's "declared income" specified, rather than deferred? [Completeness, Spec §FR-011, §Assumptions]
- [ ] CHK003 Are contribution-period requirements (length, start/end boundaries, rollover, default cycle) fully specified? [Completeness, Spec §Assumptions]
...
- [ ] CHK005 Are requirements defined for profile dissolution / what happens to pooled funds and shared elements when the last member or the owner exits? [Completeness, Gap, Spec §FR-007a]
...
- [ ] CHK009 Is "declared income" qualified with basis and frequency (gross vs net, per-period vs annual)? [Clarity, Spec §FR-011]
...
- [ ] CHK013 Is the effective-period boundary of a redistribution (when "future" begins relative to the current period) explicitly defined? [Clarity, Ambiguity, Spec §FR-016]
- [ ] CHK014 Is the immutability scope of "historical contribution records" defined (which fields are frozen, which may annotate)? [Clarity, Spec §FR-016, §SC-006]
...
- [ ] CHK023 Is SC-007's "at any time" bounded by any freshness/latency expectation to be testable? [Measurability, Spec §SC-007]
...
- [ ] CHK025 Are alternate-flow requirements specified for invitation decline and invite expiry? [Coverage, Alternate Flow, Gap, Spec §FR-002]
...
- [ ] CHK028 Are concurrent-edit conflict-resolution requirements defined, given the edge case is raised but no requirement exists? [Coverage, Gap, Spec §Edge Cases]
...
- [ ] CHK030 Are requirements defined for when a member's declared income changes mid-period (retroactive vs forward-only effect)? [Edge Case, Gap, Spec §FR-016]
...
- [ ] CHK039 Are membership state transitions (invited → active → left, and ownership held → transferred) enumerated as requirements? [Gap, Spec §FR-002, §FR-007a]
...
- [ ] CHK042 Is the deferral of the income-source mechanism flagged as a planning risk with an owner/decision point? [Assumption, Spec §Assumptions]
...
- Highest-risk open ambiguities flagged for the author: CHK029 (overpayment rule), CHK038
  (released vs reassigned responsibility), CHK037 (ownership transfer mechanics).
