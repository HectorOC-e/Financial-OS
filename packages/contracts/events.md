...
| `MemberInvited` | Membership | `userId, role, invitationExpiresAt` | Notification consumer |
| `InvitationAccepted` | Membership | `userId` | Recompute pool; audit |
| `InvitationDeclined` | Membership | `userId` | Audit (no access granted) — FR-002a |
| `InvitationExpired` | Membership | `userId` | Auto-transition after 14d; audit — FR-002a |
| `DeclaredIncomeSet` | Membership | `amountCents` | Affects NEXT period snapshot only — FR-011/FR-016a |
| `ProfileArchived` | SharedProfile | `archivedAt` | Freeze writes; read-only — FR-007c |
